...
const ProductFilterModal: FC<ProductFilterModalProps> = ({
	...
	// Obtener solo las categorías de productos
	const productCategories = catalogCategoriesMock.filter(
		(category) => category.type === EnumCatalogType.Product
	);
	...
	const productPrices = catalogProductsMock.map((product) => product.price);
	const minPossiblePrice = Math.min(...productPrices);
	const maxPossiblePrice = Math.max(...productPrices);
	...
	const generateHistogram = () => {
		...
		// Contar productos en cada rango de precio
		...
		return histogram.map((count, index) => {
			...
			// Todas las barras mantienen altura visual, independiente de si tienen productos
	...
	const getSelectedRangeStats = () => {
		const [selectedMin, selectedMax] = priceRange;
		const productsInRange = catalogProductsMock.filter(
			(product) => product.price >= selectedMin && product.price <= selectedMax
		);

		return {
			count: productsInRange.length,
			total: catalogProductsMock.length,
			percentage: Math.round(
				(productsInRange.length / catalogProductsMock.length) * 100
			),
		};
	};
			...
			<ModalContent>
				<ModalHeader className="flex flex-col gap-1">
					<h2 className="text-xl font-semibold">
						{t("catalog_filters_title", "Filtros de Productos")}
					</h2>
					<p className="text-sm text-default-500">
						{t(
							"catalog_filters_description",
							"Filtra los productos por categoría y rango de precio"
						)}
					</p>
				</ModalHeader>
						...
						<div className="flex flex-col gap-1">
							...
							<p className="text-xs text-default-500">
								{t(
									...
									"Precio del producto (incluye todas las tarifas)"
								)}
							</p>
						</div>
